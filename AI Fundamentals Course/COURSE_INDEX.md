# AI Fundamentals Course - Complete Course Index
## Proxiant Academy | 2026 Edition

**Course Level:** Foundational to Intermediate  
**Duration:** 4 Weeks (Intensive)  
**Enrollment Fee:** $1,499  
**Certification:** PCAIP (Proxiant Certified AI Professional)

---

## Course Overview

AI Fundamentals is a comprehensive 4-week intensive course designed to provide a solid understanding of artificial intelligence concepts, techniques, and applications. The curriculum bridges theoretical foundations with practical implementations, making AI accessible to both technical and non-technical audiences.

### Course Goals
- Master AI/ML/DL fundamentals and their relationships
- Understand core problem-solving algorithms and search methods
- Build practical machine learning and deep learning models
- Explore generative AI, ethics, and strategic AI adoption
- Qualify for industry-recognized PCAIP certification

### Learning Outcomes
Upon successful completion, students will be able to:
1. Define AI/ML/DL and articulate their relationships
2. Explain AI history from origins to modern breakthroughs
3. Identify core AI problem-solving methods and algorithms
4. Distinguish between supervised, unsupervised, and reinforcement learning
5. Understand neural network architectures (CNNs, RNNs, Transformers)
6. Evaluate AI models using proper metrics and validation
7. Describe real-world AI applications across industries
8. Understand LLMs and Generative AI capabilities
9. Analyze ethical implications of AI systems
10. Develop strategic perspective on AI adoption

---

## PCAIP Certification Details

**Proxiant Certified AI Professional (PCAIP)**

| Attribute | Details |
|-----------|---------|
| Exam Format | 100 Multiple Choice Questions (MCQs) |
| Duration | 3 hours |
| Passing Score | 70% (70 out of 100 questions) |
| Exam Fee | $199 |
| Certificate Validity | 3 years from certification date |
| Credential Type | Digital badge + verifiable online credential |
| Eligibility | Pass AI Fundamentals with 70% or higher |

**Career Value:**
- Industry-recognized credential validating AI expertise
- Shareable digital badge for professional profiles
- Verifiable certification through online credential system
- Foundation for advanced AI certifications

---

## Assessment Structure

### Continuous Evaluation (65%)
- **Weekly Exercises (15%):** In-class and take-home practice problems
- **Weekly Assignments (20%):** Graded homework covering weekly content
- **Weekly Projects (30%):** Hands-on practical application projects

### Final Assessment (35%)
- **Final Certification Exam:** Comprehensive 4-week assessment
- **Format:** 100 multiple choice questions
- **Passing Score:** 70% overall weighted average

**Note:** Passing score of 70% qualifies students for PCAIP certification exam.

---

## Course Materials Inventory

### Course-Level Documents (6 files)

1. **AI_Fundamentals_Course_Catalogue.docx** (23 KB)
   - Course overview and structure
   - Target audience and learning objectives
   - Weekly topic breakdown
   - Assessment and certification details
   - PCAIP specification table
   - Enrollment and exam pricing

2. **AI_Fundamentals_Syllabus.docx** (22 KB)
   - Complete course syllabus
   - Detailed learning outcomes
   - Weekly schedule breakdown
   - Course policies and expectations
   - PCAIP certification section
   - Required resources and references

3. **Final_Certification_Test.docx** (19 KB)
   - Comprehensive 4-week final exam
   - Certification assessment material

4. **Final_Certification_Test_Solution.docx** (26 KB)
   - Complete answer key
   - Detailed solutions and explanations

5. **AI_Fundamentals_Course_Catalogue.pdf** (80 KB)
   - PDF edition of the course catalogue

6. **AI_Fundamentals_Syllabus.pdf** (193 KB)
   - PDF edition of the course syllabus

---

## Weekly Breakdown

### Week 1: Foundations of Artificial Intelligence
**Topic:** Understanding what AI is, its history, and core problem-solving approaches

**Materials (8 files, ~192 KB):**
- Week1_Slides_Foundations_of_AI.pptx (106 KB) - Lecture slides
- Week1_Class_Notes.docx (18 KB) - Detailed notes
- Week1_Exercises.docx (13 KB) + Week1_Exercise_Solutions.docx (13 KB)
- Week1_Assignment.docx (9.0 KB) + Week1_Assignment_Solution.docx (11 KB)
- Week1_Project.docx (9.7 KB) + Week1_Project_Solution.docx (12 KB)

**Topics Covered:**
- What is AI? History, definitions, Turing Test
- Types of AI (Narrow, General, Super)
- AI vs Machine Learning vs Deep Learning
- Problem-solving as search
- Search algorithms (BFS, DFS, A*)
- Knowledge representation and logic
- Intelligent agents and environments

---

### Week 2: Machine Learning Fundamentals
**Topic:** Core ML paradigms and techniques for learning from data

**Materials (8 files, ~142 KB):**
- Week2_Slides_Machine_Learning.pptx (75 KB) - Lecture slides
- Week2_Class_Notes.docx (13 KB) - Detailed notes
- Week2_Exercises.docx (10 KB) + Week2_Exercise_Solutions.docx (10 KB)
- Week2_Assignment.docx (9.5 KB) + Week2_Assignment_Solution.docx (9.5 KB)
- Week2_Project.docx (8.1 KB) + Week2_Project_Solution.docx (7.8 KB)

**Topics Covered:**
- ML pipeline and paradigms
- Supervised learning (regression, classification)
- Linear and logistic regression
- Gradient descent optimization
- Decision trees and ensemble methods
- Support Vector Machines (SVMs)
- Unsupervised learning and clustering
- Model evaluation and cross-validation
- Bias-variance tradeoff

---

### Week 3: Deep Learning and Neural Networks
**Topic:** Neural network architectures and applications

**Materials (8 files, ~185 KB):**
- Week3_Slides_Deep_Learning.pptx (119 KB) - Lecture slides
- Week3_Class_Notes.docx (9.8 KB) - Detailed notes
- Week3_Exercises.docx (9.2 KB) + Week3_Exercise_Solutions.docx (9.1 KB)
- Week3_Assignment.docx (9.3 KB) + Week3_Assignment_Solution.docx (8.6 KB)
- Week3_Project.docx (9.3 KB) + Week3_Project_Solution.docx (10 KB)

**Topics Covered:**
- Neural network fundamentals
- Perceptrons and activation functions
- Backpropagation and training
- Multi-layer neural networks
- Convolutional Neural Networks (CNNs)
- Recurrent Neural Networks (RNNs)
- Long Short-Term Memory (LSTMs)
- Transformer architecture
- Attention mechanisms
- Transfer learning

---

### Week 4: Generative AI, Ethics, and AI Strategy
**Topic:** Modern AI applications, responsible AI, and organizational strategy

**Materials (8 files, ~197 KB):**
- Week4_Slides_GenAI_Ethics_Strategy.pptx (112 KB) - Lecture slides
- Week4_Class_Notes.docx (15 KB) - Detailed notes
- Week4_Exercises.docx (12 KB) + Week4_Exercise_Solutions.docx (12 KB)
- Week4_Assignment.docx (11 KB) + Week4_Assignment_Solution.docx (12 KB)
- Week4_Project.docx (10 KB) + Week4_Project_Solution.docx (12 KB)

**Topics Covered:**
- Large Language Models (LLMs)
- Generative AI fundamentals
- Prompt engineering techniques
- AI ethics and bias mitigation
- Fairness and transparency (XAI)
- AI safety and alignment
- Privacy and regulatory landscape
- AI applications across industries
- Building AI strategy and roadmaps
- PCAIP certification preparation

---

## Complete File Listing

### Directory Structure
```
AI-Course-Material/AI Fundamentals Course/
├── AI_Fundamentals_Course_Catalogue.docx
├── AI_Fundamentals_Course_Catalogue.pdf
├── AI_Fundamentals_Syllabus.docx
├── AI_Fundamentals_Syllabus.pdf
├── Final_Certification_Test.docx
├── Final_Certification_Test_Solution.docx
├── Individual feedback Form -Team6.doc
├── COURSE_INDEX.md (this file)
├── Week 1/
│   ├── Week1_Slides_Foundations_of_AI.pptx
│   ├── Week1_Class_Notes.docx
│   ├── Week1_Exercises.docx
│   ├── Week1_Exercise_Solutions.docx
│   ├── Week1_Assignment.docx
│   ├── Week1_Assignment_Solution.docx
│   ├── Week1_Project.docx
│   └── Week1_Project_Solution.docx
├── Week 2/
│   ├── Week2_Slides_Machine_Learning.pptx
│   ├── Week2_Class_Notes.docx
│   ├── Week2_Exercises.docx
│   ├── Week2_Exercise_Solutions.docx
│   ├── Week2_Assignment.docx
│   ├── Week2_Assignment_Solution.docx
│   ├── Week2_Project.docx
│   └── Week2_Project_Solution.docx
├── Week 3/
│   ├── Week3_Slides_Deep_Learning.pptx
│   ├── Week3_Class_Notes.docx
│   ├── Week3_Exercises.docx
│   ├── Week3_Exercise_Solutions.docx
│   ├── Week3_Assignment.docx
│   ├── Week3_Assignment_Solution.docx
│   ├── Week3_Project.docx
│   └── Week3_Project_Solution.docx
└── Week 4/
    ├── Week4_Slides_GenAI_Ethics_Strategy.pptx
    ├── Week4_Class_Notes.docx
    ├── Week4_Exercises.docx
    ├── Week4_Exercise_Solutions.docx
    ├── Week4_Assignment.docx
    ├── Week4_Assignment_Solution.docx
    ├── Week4_Project.docx
    └── Week4_Project_Solution.docx
```

### Summary Statistics
- **Total Files:** 40 (including this index)
- **Total Directories:** 5
- **DOCX Documents:** 32
- **PPTX Presentations:** 4
- **PDF Documents:** 2
- **Markdown Files:** 1
- **Legacy DOC Files:** 1
- **Total Storage:** ~1.1 MB

---

## How to Use These Materials

### For Instructors
1. Start with **AI_Fundamentals_Syllabus.docx** for course policies
2. Use **Week#_Slides** for lecture preparation
3. Distribute **Week#_Class_Notes.docx** to students
4. Assign **Week#_Exercises.docx** for in-class activities
5. Collect **Week#_Assignment.docx** for grading
6. Guide **Week#_Project.docx** as capstone work
7. Administer **Final_Certification_Test.docx** in Week 4
8. Use **Final_Certification_Test_Solution.docx** for grading

### For Students
1. Review **AI_Fundamentals_Course_Catalogue.docx** for overview
2. Read course policies in **AI_Fundamentals_Syllabus.docx**
3. Attend lectures and review **Week#_Slides**
4. Study **Week#_Class_Notes.docx** for deep learning
5. Complete **Week#_Exercises.docx** for practice
6. Submit **Week#_Assignment.docx** for evaluation
7. Develop **Week#_Project.docx** for hands-on experience
8. Review all materials before **Final_Certification_Test.docx**
9. Upon passing, pursue PCAIP certification exam

### For PCAIP Preparation
1. Review PCAIP details in catalogue and syllabus
2. Study all four weeks comprehensively
3. Complete all exercises, assignments, and projects
4. Practice with final certification test
5. Register for PCAIP exam upon course completion (70%+ passing grade)
6. Use all course materials for exam review

---

## Key Dates and Milestones

| Week | Deliverables | Milestone |
|------|--------------|-----------|
| Week 1 | All Week 1 exercises, Assignment 1, Project 1 | Foundations mastery |
| Week 2 | All Week 2 exercises, Assignment 2, Project 2 | ML fundamentals |
| Week 3 | All Week 3 exercises, Assignment 3, Project 3 | Deep learning |
| Week 4 | All Week 4 exercises, Assignment 4, Project 4, Final Exam | PCAIP eligibility |

---

## Curriculum Benchmarks

This course is informed by:
- **MIT 6.034** - Artificial Intelligence
- **Stanford CS221** - Principles and Techniques of AI
- **Carnegie Mellon University** - BS in AI Program

The curriculum combines:
- Rigorous theoretical foundations
- Hands-on practical applications
- Real-world case studies
- Industry-relevant tools and techniques

---

## Contact and Support

**Course Institution:** Proxiant Academy  
**Course Website:** proxiant.ai/academy  
**Copyright:** 2026 Proxiant Academy. All rights reserved.

For course enrollment or inquiries, visit the Proxiant Academy website.

---

## 2026 Updates

**Recent Updates to This Course:**
- Updated branding to "Proxiant Academy"
- Integrated PCAIP certification details and specifications
- Added enrollment fee: $1,499
- Added exam cost: $199
- Updated all copyright notices to 2026

**Document Versions:**
- AI_Fundamentals_Course_Catalogue.docx v2.0 (PCAIP-integrated)
- AI_Fundamentals_Syllabus.docx v2.0 (PCAIP-integrated)
- All week materials current as of February 2026

---

**Last Updated:** February 14, 2026  
**Next Review:** July 2026

